...
## 8. Firebase Hosting 워크플로 (`deploy-firebase-hosting.yml`) 실패 시

이 저장소의 `firebase.json` 은 **Hosting → Cloud Run(`tax-automation`) 리라이트**를 포함합니다. 다음을 순서대로 확인하세요.

1. **Firebase 요금제:** Cloud Run 연동 리라이트는 **Blaze(종량제)** 프로젝트에서만 허용되는 경우가 많습니다. Firebase Console → 프로젝트 → **요금제 업그레이드** 여부를 확인하세요.
2. **배포 주체 IAM:** GitHub Secret `GCP_SA_KEY` 의 `client_email` 서비스 계정에 다음을 부여합니다(콘솔 IAM 또는 gcloud).
   - `roles/firebasehosting.admin` (또는 동등한 Hosting 배포 권한)
   - Cloud Run 백엔드 검증이 필요하면 `roles/run.viewer` 추가
3. **(선택) `FIREBASE_SERVICE_ACCOUNT`:** 예전 액션 전용 시크릿이 **깨진 JSON**이면 `google-github-actions/auth` 가 1초 만에 실패합니다. 현재 워크플로는 검증된 `GCP_SA_KEY` 로 통일했습니다.

배포 로그는 Actions → **Deploy Firebase Hosting** → **Deploy to Firebase Hosting** 단계의 영문 메시지를 기준으로 원인을 좁히면 됩니다.
