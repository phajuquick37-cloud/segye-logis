...
## 9. 로그에 `4001` 또는 “공동인증서 비밀번호가 일치하지 않습니다”

**원인:** 홈택스·공동인증 연동 쪽에서 **등록된 인증서 암호가 실제 공동인증서 암호와 다르다**고 판단한 경우입니다. (흔한 표기: **결과코드 4001**.)

**운영에서 할 일:**

1. 인증서 **재발급·갱신·암호 변경**이 있었는지 확인합니다.
2. 세금 수집에 참여하는 **PC 프로그램 / 별도 에이전트 / 서버**에 저장된 **공동인증서 비밀번호**를 지금 쓰는 암호로 다시 입력·저장합니다.
3. 이 저장소의 Cloud Run 서비스(`tax-automation`)는 기본적으로 **메일 → 링크를 브라우저로 연 뒤** 처리합니다. 링크가 곧바로 **홈택스 인증서 로그인**을 요구하면, **무인(headless) 환경에서는 통과가 어려울 수** 있습니다. 그럴 경우 **원빌·화물맨·원콜 등 메일 뷰어 URL**처럼 인증서 없이 열리는 흐름이 오는지 메일·`TAX_IMAP_*` 설정을 점검합니다.

Cloud Run 로그에 `홈택스/공동인증 실패`가 찍히면 위와 동일하게 **인증서·암호·유효기간**을 우선 확인합니다.
